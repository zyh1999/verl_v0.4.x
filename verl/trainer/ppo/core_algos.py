...
def compute_policy_loss(
    ...
    # Default behavior (backward-compatible): keep original torch.maximum/torch.min formulation.
    ...
    if not use_importance_sampling:
        # "No-IS" style (same spirit as SB3's adv_loss_remove_ratio):
        # divide by ratio.detach() so the gradient becomes VPG-like (∝ A * ∇logπ),
        # while keeping PPO's clipping mask/structure.
